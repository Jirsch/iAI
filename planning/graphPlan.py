...
class GraphPlan(object):
    ...
    def graphPlan(self):
        ...
        while self.goalStateNotInPropLayer(
                self.graph[level].getPropositionLayer().getPropositions()) or \
                self.goalStateHasMutex(
                        self.graph[level].getPropositionLayer()):
            ...
            pgNext.expand(self.graph[
                level - 1])  #calls the expand function, which you are implementing in the PlanGraph class
            self.graph.append(
                pgNext)  #appending the new level to the plan graph

            sizeNoGood = len(
                self.noGoods[level])  #remember size of nogood table
        ...
        while (
            plan is None):  #while we didn't extract a plan successfully
            level = level + 1
            self.noGoods.append([])
            ...
            if (plan is None and self.isFixed(
                    level)):  #if failed and reached fixed point
                if sizeNoGood == len(self.noGoods[
                    level]):  #if size of nogood didn't change, means there's nothing more to do. We failed.
                    return None
                sizeNoGood = len(self.noGoods[
    ...
    def gpSearch(self, Graph, subGoals, plan, level):
        ...
        for action1 in [act for act in
                        Graph[level].getActionLayer().getActions() if
                        prop in act.getAdd()]:
            noMutex = True
            for action2 in plan:
    ...
    def isFixed(self, level):
        ...
        if len(self.graph[
            level].getPropositionLayer().getPropositions()) == len(self.graph[
                    level - 1].getPropositionLayer().getPropositions()) and \
                        len(self.graph[
                            level].getPropositionLayer().getMutexProps()) == len(
                        self.graph[
                                    level - 1].getPropositionLayer().getMutexProps()):
            return True
        return False
